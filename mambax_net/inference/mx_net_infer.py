...
def infer_loop():
    """Run MambaX-Net dual-scan inference on a T2 dataset and save predictions.

    Parses CLI args, loads the nnUNet plans/config for preprocessing
    parameters, builds a MambaXNet model, and constructs a DualScanDataset
    where the ground-truth labels (if given) are the "current" scan target
    and the previously generated AI predictions (AI_WP_path/AI_PZ_TZ_path)
    are fed to the model as the "past" scan prior. If both WP and PZ/TZ
    ground-truth label paths are provided, runs evaluation via
    ``inference_func`` and logs Dice/Hausdorff metrics and regional
    (apex/mid-gland/base) statistics to Weights & Biases. Always generates
    and saves prediction volumes/probability maps via
    ``generate_predictions``, regardless of whether ground truth is
    available.
    """
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to previous AI predictions for WP (e.g., from nnUNet)",
    )
    parser.add_argument(
        "-ai_pz",
        "--AI_PZ_TZ_path",
        type=str,
        required=True,
        help="Path to previous AI predictions for PZ/TZ (e.g., from nnUNet)",
    )
    parser.add_argument(
        "-wp",
        "--WP_label_path",
        type=str,
        required=False,
        help="Path to ground truth WP labels (optional, for evaluation)",
    )
    parser.add_argument(
        "-pz",
        "--PZ_TZ_label_path",
        type=str,
        required=False,
        help="Path to ground truth PZ/TZ labels (optional, for evaluation)",
    )
    parser.add_argument("-conf", "--config", type=str, required=True)
    parser.add_argument("-name", "--dataset_name", type=str, default="AS")
    ...
    os.environ["WANDB_NOTEBOOK_NAME"] = "mx_net_infer_full.py"
    ...
    # AI prediction paths (required for MambaX-Net dual-scan architecture)
    AI_WP_path = args.AI_WP_path
    AI_PZ_TZ_path = args.AI_PZ_TZ_path

    # Ground truth label paths - optional, only for evaluation
    ...
    model = MambaXNet(
        ...
        None,  # we don't need a separate nnUNet model here, as we're using the pretrained weights directly
    ...
    _use_tta = config.get("tta", False) and "mxnet" in config.get("tta_models", [])
    ...
    if has_masks:
        ...
        LOGGER.info(
            ...
            f"Mean Dice: {test_metrics['test_dice_mean']}, Mean WP: {test_metrics['test_wp_mean']}, "
            f"Mean PZ: {test_metrics['test_pz_mean']}, Mean TZ: {test_metrics['test_tz_mean']}"
        )

        LOGGER.info(
            f"  Std Loss: {test_metrics['test_std_loss']}, "
            f"Std Dice: {test_metrics['test_std_dice']}, Std WP: {test_metrics['test_std_wp']}, "
            f"Std PZ: {test_metrics['test_std_pz']}, Std TZ: {test_metrics['test_std_tz']}"
        )
